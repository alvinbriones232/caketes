...
class RealAjaxUploader {
	...
	function __construct($deny_ext=array())
	{
		...
		// $ext = pathinfo($this->f, PATHINFO_EXTENSION);

		$ext = end((explode(".", $this->file_name)));
		$this->file_name = md5($this->file_name.date('Y-m-d H:i:s')).'.'.$ext;
	 ...
	 */
	public function setMaxFileSize($max_file_size = '100M')
	{
	...
	{
		$this->header();
		$mssg = json_encode(array('name'=>$this->file_name, 'size'=>$this->file_size, 'status'=>$status,'info'=>$msg, 'more'=>$extra_info));
